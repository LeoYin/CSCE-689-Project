...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        video_data = np.load(sequence_path).item()
        video_name = self._extract_name(sequence_path)
        flow_data = np.load(sequence_path.rstrip('*_rgb.npy')+'_flow.npy').item()
        ...
        for i in range(start_i, len(video_data), sample_interval):
            if self.sequence_length is None or len(image_sequence) < self.sequence_length:
                ...
                flow_tensor = self.transform(Image.fromarray(np.uint8(flow_data[i])))
                if flip:
                ...
                flow_tensor=np.array(flow_tensor, dtype=np.float32)
                flow_tensor=torch.tensor(flow_tensor) 
        ...
        return image_sequence, flow_sequence, target
