...
class ConvLSTM(nn.Module):
    def __init__(
        ...
        self.lstm = LSTM(latent_dim*2, lstm_layers, hidden_dim, True, bidirectional)
    ...
    def forward(self, x1, x2):
        ...
        batch_size2, seq_length2, c2, h2, w2 = x2.shape
        x2 = x2.view(batch_size2 * seq_length2, c2, h2, w2)
        x2 = self.encoder2(x2)
        x2 = x2.view(batch_size2, seq_length2, -1)

        
        x = self.lstm(torch.cat((x1, x2), dim=2))
        if self.attention:
