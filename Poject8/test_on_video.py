...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_model", type=str, default="model_checkpoints/ConvLSTM.pth", help="Optional path to checkpoint model")
    ...
    for video in glob.glob(os.path.join(opt.video_path,'*.avi')):
        ...
        for j, frame in enumerate(tqdm.tqdm(extract_frames(video), desc="Processing frames")):
            ...
            image_tensor1 = Variable(transform(frame)).to(device)
            ...
            frame2 = cv2.resize(np.float32(frame), None, fx=scale, fy=scale)
            dense_flow = cv2.addWeighted(frame2, 1,rgb, 2, 0, dtype=cv2.CV_32F)
            ...
            with torch.no_grad():
                prediction = model(image_tensor1,image_tensor)
