...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_path", type=str, default="/Users/yinlihao/Desktop/DLFinal/data/videos", help="Path to dataset")
    parser.add_argument("--class_path", type=str, default="classes.txt", help="Path to frames")
    ...
    for epoch in range(opt.num_epochs):
        ...
        print('\n')
        print(f"--- Epoch {epoch} ---")
        for batch_i, (Xf, Xp, y) in enumerate(train_dataloader):

            if Xf.size(0) == 1:
                continue

            image_sequences = Variable(Xf.to(device), requires_grad=True)
            flow_sequence = Variable(Xp.to(device), requires_grad=True)
            ...
            # Get sequence predictions
            predictions = model(image_sequences, flow_sequences)
