    ...
    def __getitem__(self, index):
        ...
        for i in range(start_i, len(video_data), sample_interval):
            if self.sequence_length is None or len(image_sequence) < self.sequence_length:
                ...
                try:
                    pose_tensor=np.array(pose_data[i], dtype=np.float32)
                except:
                    pose_tensor=np.array(pose_data[-1], dtype=np.float32)
                pose_tensor=torch.tensor(pose_tensor) 
                pose_sequence.append(pose_tensor)
  
        image_sequence = torch.stack(image_sequence)
        pose_sequence = torch.stack(pose_sequence)
        ...
        return image_sequence, pose_sequence, target
