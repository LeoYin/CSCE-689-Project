...
class LSTM(nn.Module):
    def __init__(self, latent_dim, num_layers, hidden_dim, batch_first=True, bidirectional=True):
        super(LSTM, self).__init__()
        self.lstm = nn.LSTM(latent_dim, hidden_dim, num_layers, batch_first=batch_first, bidirectional=bidirectional)
...
class ConvLSTM(nn.Module):
    def __init__(
        ...
        self.lstm = LSTM(latent_dim+32, lstm_layers, hidden_dim, True, bidirectional)
    ...
    def forward(self, x1, x2):
        batch_size, seq_length, c, h, w = x1.shape
        x1 = x1.view(batch_size * seq_length, c, h, w)
        x1 = self.encoder(x1)
        x1 = x1.view(batch_size, seq_length, -1)

        batch_size2, seq_length2, c2, h2 = x2.shape
        x2 = x2.view(batch_size*seq_length, -1)
        x2 = self.pose_layers(x2)
        x2 = x2.view(batch_size,seq_length, -1)
        
        
        x1 = self.lstm(torch.cat((x1,x2),dim=2))
        if self.attention:
            attention_w = F.softmax(self.attention_layer(x1).squeeze(-1), dim=-1)
            x1 = torch.sum(attention_w.unsqueeze(-1) * x1, dim=1)
        else:
            x1 = x1[:, -1]
  
        return self.output_layers(x1)
