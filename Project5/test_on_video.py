...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_model", type=str, default="model_checkpoints/ConvLSTM_65.pth", help="Optional path to checkpoint model")
    ...
    for video in glob.glob(os.path.join(opt.video_path,'*.avi')):
        ...
        pose_data = np.load(video.rstrip('_rgb.avi')+'_skeleton.npy')
        ...
        for j, frame in enumerate(tqdm.tqdm(extract_frames(video), desc="Processing frames")):
            if j%skip!=0:
                ...
                continue
            try:
                pose=np.array(pose_data[j], dtype=np.float32)
            except:
                pose=np.array(pose_data[-1], dtype=np.float32)
            image_tensor = Variable(transform(frame)).to(device)
            pose_tensor = Variable(torch.tensor(pose)).to(device)
            image_tensor = image_tensor.view(1, 1, *image_tensor.shape)
            pose_tensor = pose_tensor.view(1, 1, *pose_tensor.shape)

            # Get label prediction for frame
            with torch.no_grad():
                prediction = model(image_tensor,pose_tensor)
